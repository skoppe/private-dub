...
struct GitlabConfig {
  ...
  static GitlabConfig loadConfig(IniSection section) {
    ...
    return GitlabConfig(section.getKey("token"), parts[1], section.getKey("storage",
        GitlabConfig.init.storage), section.getKey("priority",
        GitlabConfig.init.priority.to!string).to!int);
  }
}
